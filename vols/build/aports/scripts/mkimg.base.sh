...
build_apkovl() {
	...
	msg "Generating awesome $_host.apkovl.tar.gz"
...
grub_gen_config() {
	...
	for _f in $kernel_flavors; do
		if [ -z "${xen_params+set}" ]; then
			...
			menuentry "Home Assistant OS Installer - DEBUG LOGGING" {
				linux	/boot/vmlinuz-$_f ${initfs_cmdline/quiet} hass.debug=1 $kernel_cmdline
				initrd	$_initrd
			}
			EOF
		else
